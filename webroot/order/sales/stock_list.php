...
//订单中所有的sku
$listGoods                  = ArrayUtility::listField($salesOrderGoodsInfo,'goods_id');

//销售订单中的备注
$indexSales                 = ArrayUtility::indexByField($salesOrderGoodsInfo,'goods_id');

if(empty($listGoods)){
    
    Utility::notice('销售订单中没有产品','order/sales/supplies/index.php?sales_order_id='.$salesOrderId);
}
...
$template->assign('indexSales', $indexSales);
$template->assign('sumShipment', array_sum(ArrayUtility::listField($salesOrderGoodsInfo,'shipment')));

$template->assign('mapOrderStatus', $mapOrderStatus);
$template->display('order/sales/stock_list.tpl');
